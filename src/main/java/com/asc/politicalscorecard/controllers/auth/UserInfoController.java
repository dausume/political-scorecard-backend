...
public class UserInfoController {
    ...
    public ResponseEntity<ApiResponse<UserInfo>> testAuth(Authentication authentication) {
        ...
        if (userInfo != null) {
            ApiResponse<UserInfo> response = ResponseHandler.generateSuccessResponse(
                "Authentication successful! User info retrieved from JWT token", userInfo);
            return new ResponseEntity<>(response, HttpStatus.OK);
        } else {
            ApiResponse<UserInfo> response = new ApiResponse<>(
                false, "Unable to extract user information from token", null);
